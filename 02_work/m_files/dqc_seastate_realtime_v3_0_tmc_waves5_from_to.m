function [bag] = dqc_seastate_realtime_v3_0_tmc_waves5_from_to(incoming_folder, work_folder, outgoing_folder, log_folder, sensor, station, date_from, date_to, config_output, waves5_indicator) 

%% authors: Mayumi Wilms, mayumi.wilms@bsh.com, BSH
%%          Martin Moritz, martin.moritz@bsh.de, BSH
%% date: 26.02.2025
%% version: 3.0

%{
% Real-time quality control tests for sea state (DWR, Radac, Radac Single)
% Looks for the timerange (date_now - x_days) to date_now (editable)
% Problematic timestamps in *.hiw, *.his and *GPS.txt are automatically removed
% DQC for DWR with Waves21 and Waves5 (timestamps are xx:00 and xx:30 for hiw/his/gps; timestamps definition his/gps is still "end of measurement")
...
% change waves5_indicator from string to logical
if strcmp(waves5_indicator,'1')
    bag.waves5_indicator = true;
else 
    bag.waves5_indicator = false;
end
...
switch upper(bag.s_sensor)
    case 'DWR' 
        ...
        elseif contains(bag.s_station,{'SEE'}) % select NLWKN stations (WAVES4)
            [bag] = parser_dwr_nlwkn(bag);      
        elseif waves5_indicator == true % select stations with WAVES5
            [bag] = parser_dwr_waves5(bag);
        else 
            [bag] = parser_dwr(bag);
        end
...
%% remove problematic timestamps in HIS and GPS (from stations without WAVES5)
if bag.waves5_indicator == false
    [bag] = DQC_rmdate(bag);
end
