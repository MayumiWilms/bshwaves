function [bag] = parser_dwr_waves5(bag)
...
T_HIS = T_HIS(or(T_HIS.Time.Minute == 0 , T_HIS.Time.Minute == 30),:); % select timestamps with xx:00 and xx:30   
...
if isempty(dirinfo_gps_yearX)
    ...
    T_GPS = T_GPS(or(T_GPS.Time.Minute == 0 , T_GPS.Time.Minute == 30),:); % select timestamps with xx:00 and xx:30   
